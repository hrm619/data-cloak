...
# Run the project
uv run python main.py input.csv
uv run python main.py input.csv --config financial_config.toml

# Run tests
uv run pytest

# Run a single test file or class
uv run pytest test_financial.py
uv run pytest test_financial.py::TestAnonymizeDate

# Generate 10M-row test data (takes several minutes)
uv run python generate_synthetic_base.py
uv run python inject_messiness.py

# Run volume/performance tests (requires generated test data)
uv run pytest test_volume_performance.py -v
```

## Architecture

All code lives in the worktree at `.claude/worktrees/gifted-goldberg/`.

**`anonymize.py`** — core anonymization logic, layered as atomic functions:
...
**`main.py`** — CLI entry point. Orchestrates `load_config` → `read_csv` → `anonymize_dataframe` → `write_csv`. Passes input filename for deterministic offset/multiplier derivation.
